...
struct synchronized_port_v00_linux_t : public synchronized_port_t<buffers_ring_v00_linux_t> {
    std::pair<
        std::shared_ptr<initializer_memfd_t>,
        std::shared_ptr<buffers_ring_v00_linux_t>
    > m_mem_buf;
    synchronized_port_v00_linux_t(std::pair<std::shared_ptr<initializer_memfd_t>,std::shared_ptr<buffers_ring_v00_linux_t>> mem_buf)
        : m_mem_buf(mem_buf), synchronized_port_t<buffers_ring_v00_linux_t>(mem_buf.second->get_first()) {}
};
